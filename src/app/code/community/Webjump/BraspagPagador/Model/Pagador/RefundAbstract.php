 ...
 * @link      http://www.webjump.com.br
 **/
abstract class Webjump_BraspagPagador_Model_Pagador_RefundAbstract
{
    protected $payment;
    ...
    public function execute($payment)
    {
        try {
            ...
            $transaction = $this->getServiceManager()->get('Pagador\Transaction\Refund');
    ...
    protected function processResponse($response, $payment)
    {
        ...
        $transactionId = empty($payment->getTransactionId()) ? $payment->getParentTransactionId() : $payment->getTransactionId();

        $parentTransactionId = str_replace('-capture', '', $transactionId);
    ...
    protected function saveInfoData($paymentDataResponse, $payment)
    {
        ...
        $info->setAdditionalInformation('refunded_total', $paymentDataResponse->getAmount());
