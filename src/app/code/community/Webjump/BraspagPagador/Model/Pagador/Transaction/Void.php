 ...
 * @link      http://www.webjump.com.br
 **/
class Webjump_BraspagPagador_Model_Pagador_Transaction_Void extends Webjump_BraspagPagador_Model_Pagador_Transaction_Abstract
{
    ...
    /*
        O recurso de void parcial não esta
    ...
    */
    ...
    public function processResponse($response)
    {
        ...
        if (!$this->getParentTransactionId()) {
            Mage::throwException(Mage::helper('webjump_braspag_pagador')->__('Authorize Transaction is necessary to void this transaction'));
        }

        $this->proccessOrderState($response);
        $this->proccesssOrderVoidTransaction($response);
    ...
    {
        $message = Mage::helper('sales')->__('Voided authorization. Amount: %s.', $this->_formatPrice(
    ...
    protected function proccesssOrderVoidTransaction($response)
    {
    ...
    protected function convertResponseToTransactiondetails($response)
    {
       ...
       foreach ($response['transactions'] as $transacionKey => $transaction) {
            foreach ($transaction as $key => $data) {
                $transactionDetails['void_' . $transacionKey . '_' . $key ] = $data;
            }
       }
