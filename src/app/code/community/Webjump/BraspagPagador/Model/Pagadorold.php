 ...
 * @link      http://www.webjump.com.br
 **/
class Webjump_BraspagPagador_Model_Pagadorold extends Mage_Core_Model_Abstract
{
     ...
     * @throws Exception
     */
    public function capture(Varien_Object $payment, $captureData)
    {
    	try{
    		...
    		$result = $api->capture($payment, $captureData);
			if ($errors = $result->getErrorReport()->getErrors()) {
                foreach ($errors AS $error) {
                    if (empty($error)) {
                        continue;
    ...
    public function void(Varien_Object $payment)
    {
    	try{
    		...
    		if (!method_exists($api, 'void')) {
    			throw new Exception(Mage::helper('webjump_braspag_pagador')->__('Void is not available at payment api.'));
    		}
    		
    		$result = $this->getApi($payment)->void($payment);
	        return $result;
		} catch (Exception $e) {
