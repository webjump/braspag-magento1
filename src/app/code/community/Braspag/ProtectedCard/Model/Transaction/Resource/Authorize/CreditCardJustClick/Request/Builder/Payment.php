 ...
 * @link      http://www.webjump.com.br
 **/
class Braspag_ProtectedCard_Model_Transaction_Resource_Authorize_CreditCardJustClick_Request_Builder_Payment
extends Braspag_Pagador_Model_Transaction_Resource_Authorize_CreditCard_Request_Builder_Payment
{
    ...
    {
        parent::build($payment, $amount);

        $paymentData = $this->getNewVarienObject();
        $paymentData->addData(parent::getDataBuild());

        $paymentCompositeData = $this->getBraspagCoreConfigHelper()
            ->getDefaultConfigClassComposite('braspag_pagador/transaction/command/authorize/credit_card_just_click/request/builder/composite/payment');
        ...
        $creditCardPayment = $this->getServiceManager()->get('Pagador\Data\Request\Payment\CreditCardJustClick');
