 ...
 * @link      http://www.webjump.com.br
 **/
class Braspag_PaymentSplit_Model_Transaction_Resource_Authorize_CreditCard_Request_Builder_Payment_SplitPayments
    extends Braspag_Pagador_Model_Transaction_Resource_Authorize_CreditCard_Request_Builder_Payment
{
    ...
    public function build($payment, $amount, $compositeDataObject)
    {
        ...
        if (!$splitPaymentConfigModel->isActive()
            || $payment->getMethodInstance()->getConfigData('payment_action') == 'authorize'
            || (
                $payment->getMethodInstance()->getConfigData('payment_action') == 'authorize_capture'
        ...
        foreach ($paymentSplitData->getSubordinates() as $subordinate) {
            ...
            $paymentSplitFareRequestData = $this->getServiceManager()->get('Pagador\Data\Request\Payment\CreditCard\SplitPayment\Fare');
            $paymentSplitFareRequestData->populate($subordinate->getFares()->getData());

            $paymentSplitRequestData = $this->getServiceManager()->get('Pagador\Data\Request\Payment\CreditCard\SplitPayment');
