 ...
 * @link      http://www.webjump.com.br
 **/
class Braspag_Pagador_Model_Transaction_Resource_Capture_CreditCard_Response_Processor
extends Braspag_Pagador_Model_Transaction_AbstractProcessor
{
    ...
    public function process($payment, $response)
    {
        ...
        if (!$paymentDataResponse) {
            \Mage::throwException('The payment was uncaptured.');
        }

        $transactionId = $this->getBraspagPagadorHelper()->cleanTransactionId($payment->getLastTransId());

        $payment
            ->setTransactionParentId($transactionId)
            ->setTransactionId($transactionId.'-capture')
            ->setIsTransactionClosed(1);

        $processes = $this->getBraspagCoreConfigHelper()
            ->getDefaultConfigClassComposite('braspag_pagador/transaction/command/capture/credit_card/response/processor');
