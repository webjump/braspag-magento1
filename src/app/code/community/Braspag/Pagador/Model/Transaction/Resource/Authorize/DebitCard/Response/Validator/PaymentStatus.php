 ...
 * @link      http://www.webjump.com.br
 **/
class Braspag_Pagador_Model_Transaction_Resource_Authorize_DebitCard_Response_Validator_PaymentStatus
extends Braspag_Pagador_Model_Transaction_Resource_Authorize_DebitCard_Response_Validator
{
    /**
     * @param $dataObject
     * @return bool
    ...
    public function isValid($dataObject)
    {
        ...
        if (in_array($payment->getStatus(), [
            ...
            || (!$payment->getAuthenticationUrl() && $payment->getStatus()
                == BrasPag_Lib_Pagador_TransactionInterface::TRANSACTION_STATUS_NOT_FINISHED
            )
        ) {
            $this->validatorMessages[] = $this->getBraspagPagadorHelper()->__("(Code {$payment->getProviderReturnCode()}) "
