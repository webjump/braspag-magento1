 ...
 * @link      http://www.webjump.com.br
 **/
class Braspag_Pagador_Model_Transaction_Resource_Refund_CreditCard_Request_Handler
    extends Braspag_Pagador_Model_Transaction_AbstractHandler
{
    ...
    public function handle($payment, $request)
    {
        $handlers = $this->getBraspagCoreConfigHelper()
            ->getDefaultConfigClassComposite('braspag_pagador/transaction/command/refund/credit_card/request/handler');
        ...
        $refundTransaction = $this->getServiceManager()->get('Pagador\Transaction\Refund');

        $refundTransaction->execute($request);

        if (Mage::getSingleton('braspag_core/config_general')->isDebugEnabled()) {

            Mage::getSingleton('braspag_pagador/transaction')
                ->debugTransaction($payment, $refundTransaction);
        }
