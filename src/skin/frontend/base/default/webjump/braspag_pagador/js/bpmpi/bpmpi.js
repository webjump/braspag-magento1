...
Bpmpi.prototype = {
  ...
  placeOrder: function () {
    ...
    $j('.bpmpi_auth_failure_type').change(function () {
      ...
      if (self.isBpmpiEnabled()) {
        ...
        if (self.paymentType == 'creditcard') {
          ...
          var paymentForm = $j("#" + payment.form + ' #webjump_braspag_cc_0_cc_type_div');
        } else if (self.paymentType == 'debitcard') {

          var paymentForm = $j("#" + payment.form + ' #webjump_braspag_dc_0_dc_type_div');
        } else {
        ...
        self.bpmpiRenderer.createInputHiddenElement(
          paymentForm, 'payment[payment_request][0][authentication_failure_type]', 'authentication_failure_type', ''
        );

        self.bpmpiRenderer.createInputHiddenElement(
          paymentForm, 'payment[payment_request][0][authentication_cavv]', 'authentication_cavv', ''
        );

        self.bpmpiRenderer.createInputHiddenElement(
          paymentForm, 'payment[payment_request][0][authentication_xid]', 'authentication_xid', ''
        );

        self.bpmpiRenderer.createInputHiddenElement(
          paymentForm, 'payment[payment_request][0][authentication_eci]', 'authentication_eci', ''
        );

        self.bpmpiRenderer.createInputHiddenElement(
          paymentForm, 'payment[payment_request][0][authentication_version]', 'authentication_version', ''
        );

        self.bpmpiRenderer.createInputHiddenElement(
          paymentForm, 'payment[payment_request][0][authentication_reference_id]', 'authentication_reference_id', ''
        );
    ...
    if (paymentMethod == 'webjump_braspag_cc') {
      this.paymentType = 'creditcard';

    } else if (paymentMethod == 'webjump_braspag_dc') {
  ...
  validateAuthenticate: function() {
    ...
    if (self.paymentType == 'creditcard'){
      var providerCC = $j('#webjump_braspag_cc_0_cc_type').val();
    ...
    if (self.paymentType == 'debitcard') {
      var providerDC = $j('#webjump_braspag_dc_0_dc_type').val();
  ...
  renderData: function () {
    ...
    return new Promise(function(resolve, reject){
      new Ajax.Request('/braspag_pagador/mpi/authenticateData',
        {
          method:'get',
  ...
  renderCredicardData: function() {
    ...
    this.bpmpiRenderer.renderBpmpiData('bpmpi_cardnumber', false, $j('#webjump_braspag_cc_0_cc_number').val());
    this.bpmpiRenderer.renderBpmpiData('bpmpi_billto_contactname', false, $j('#webjump_braspag_cc_0_cc_owner').val());
    this.bpmpiRenderer.renderBpmpiData('bpmpi_cardexpirationmonth', false, $j('#webjump_braspag_cc_0_expiration').val());
    this.bpmpiRenderer.renderBpmpiData('bpmpi_cardexpirationyear', false, $j('#webjump_braspag_cc_0_expiration_yr').val());
    this.bpmpiRenderer.renderBpmpiData('bpmpi_installments', false, $j('#webjump_braspag_cc_0_installments').val());
  },
  ...
  renderDebitcardData: function() {
    ...
    this.bpmpiRenderer.renderBpmpiData('bpmpi_cardnumber', false, $j('#webjump_braspag_dc_0_dc_number').val());
    this.bpmpiRenderer.renderBpmpiData('bpmpi_billto_contactname', false, $j('#webjump_braspag_dc_0_dc_owner').val());
    this.bpmpiRenderer.renderBpmpiData('bpmpi_cardexpirationmonth', false, $j('#webjump_braspag_dc_0_expiration').val());
    this.bpmpiRenderer.renderBpmpiData('bpmpi_cardexpirationyear', false, $j('#webjump_braspag_dc_0_expiration_yr').val());
